...
%% ***** Preprocessing *****
subName = 'sub03';
rootDir    = 'P:\3013066.01\raw\subjects_nii_files'; % directory where your subject folders are stored
epiDir = fullfile(rootDir,subName, 'MRI1','TASK'); %assumend folder names in every folder, change if different
epiFileArray = spm_select('ExtFPList', epiDir,'^vol.*\.nii$'); % CHANGE THIS!!!
anatomyDir = fullfile(rootDir,subName,'MRI1','T1');   %assumed name of the folder in which the T1 file is stored, change if necessary

tPMFile    =  fullfile('H:\common\matlab', 'spm12', 'tpm', 'TPM.nii');  % Possibly adjust to own path of SPM install
anatomyFile = spm_select('ExtFPList', anatomyDir,'2017.*\.nii$');         % adjust to how your images are called
...
matlabbatch{3}.spm.spatial.coreg.estimate.source = []; %{anatomyFile}; %cfg_dep('Move/Delete Files: Moved/Copied Files', substruct('.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('.','files'));
